...
function payam_render_faq_items( array $items, bool $open_first = true ): string {
	...
	foreach ( $items as $index => $item ) {
		...
		<article class="faq-item<?= $is_open ? ' is-open' : ''; ?> shadow-mellow" data-faq-item data-faq-categories="<?= esc_attr( implode( ' ', $item['categories'] ?? [] ) ); ?>">
			...
			<div class="faq-answer" id="<?= esc_attr( $panel_id ); ?>" data-faq-panel role="region" aria-labelledby="<?= esc_attr( $button_id ); ?>"<?= $is_open ? '' : ' hidden'; ?>><div class="faq-answer-content text-description md:text-caption text-caption-mobile"><?= wp_kses_post( $item['answer'] ?? '' ); ?></div></div>
		</article><?php
	}
