...
$contactItems = array_values(
	array_filter(
		$contactItems,
		static fn( $item ): bool => is_array( $item ) && ! empty( $item['value'] )
	)
);
...
<footer class="relative overflow-hidden bg-dark pt-16 text-white xl:pt-[107px]" data-footer>
	<img class="pointer-events-none absolute inset-0 h-full w-full object-cover" src="<?= esc_url( $footerAsset( 'footer-background.webp' ) ); ?>" alt="" width="1440" height="800" loading="lazy" decoding="async">
	...
	<div class="container relative z-10 flex flex-col items-center">
		...
		<div class="mt-[136px] grid w-full grid-cols-1 gap-4 xl:mt-24 xl:grid-cols-5 xl:items-start xl:gap-12" data-footer-accordion>
		...
		<?php if ( $footerCopyright ) : ?>
			<div class="mt-8 w-full border-t border-white/10 pt-4 text-right text-xs leading-[30px] text-white xl:mt-10 xl:text-center xl:text-sm [&_a]:text-yellow-primary text-[14px] font-normal">
