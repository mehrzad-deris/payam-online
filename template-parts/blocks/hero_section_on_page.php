...
<section
        class="hero-section-on-page <?= 'dark' === $sectionStyle ? 'text-white' : ''; ?> min-h-screen bottom-fade relative overflow-hidden"
...
>
    <div class="container relative z-2 pb-16 pt-39 lg:pt-43">
        <?php if ( '' !== $sectionTitle ) : ?>
        <<?= esc_attr( $sectionTitleTag ); ?> class="mb-4 whitespace-pre-line text-center text-[24px] font-bold leading-12 lg:text-[36px] lg:leading-14">
    ...
    <?php if ( ! empty( $sectionServices ) ) : ?>
        <div class="server-tabs mt-5" data-tabs data-tabs-mobile="tabs">
                ...
                <div class="server-tab-list-inner">
                    <?php foreach ( $sectionServices as $index => $serviceItem ) :
                        ...
                        <button
                                ...
                                data-server-type="<?= esc_attr( $tabType ); ?>"
                        >
                            <?= esc_html( $tabTitle ); ?>
            ...
            <div class="server-tabs-panels">
                <?php foreach ( $sectionServices

                as $index => $serviceItem ) :
                $tabType      = sanitize_key( $serviceItem['service_type'] ?? $serviceItem['server_type'] ?? $serviceItem['type'] ?? $availableServerTypes[ $index ] ?? '' );
                $tabId        = $tabsId . '-tab-' . $index;
                $panelId      = $tabsId . '-panel-' . $index;
                ...
                >
                    <div class="server-list pt-10">
                        ...
                        <article
                                ...
                                <?php if ( $serverId ) : ?>
                                    data-server-id="<?= esc_attr( $serverId ); ?>"
                                <?php endif; ?>
                        >
                            <?php if ( '' !== $serverUrl ) : ?>
                            <a
                                    href="<?= esc_url( $serverUrl ); ?>"
                            ...
                            >
                                ...
                                <div class="server-card-inner">
                                    ...
                                    <header class="server-card-header">
                                        ...
                                        >
                                            <?= esc_html( $serverTitle ); ?>
                                        </h3>

                                        <?php if ( '' !== $serverItem['desc'] ) : ?>
                                            <p class="server-card-description">
                                                <?= esc_html( $serverItem['desc'] ); ?>
                                            </p>
                                        <?php endif; ?>
                                    </header>
                                    ...
                                    <?php if ( ! empty( $serverItem['config'] ) ) : ?>
                                        <dl class="server-card-config">
                                            <?php foreach ( $serverItem['config'] as $config ) :
                                                if ( ! is_array( $config ) ) {
                                                    continue;
                                                }
							...
							<?= esc_html( $periodLabel ); ?>
                                    ...
                                    <?php if ( '' !== $serverUrl ) : ?>
                                        <span class="server-card-link" aria-hidden="true">
					مشاهده و خرید
				</span>
                                    <?php endif; ?>
